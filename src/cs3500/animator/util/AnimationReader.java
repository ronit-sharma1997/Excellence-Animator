...
public class AnimationReader {
  ...
  private static <Doc> void readMotion(Scanner s, AnimationBuilder<Doc> builder) {
    boolean containsRotation = true;
    String[] fieldNames = new String[]{
        "initial time",
        "initial x-coordinate", "initial y-coordinate",
        "initial width", "initial height",
        "initial red value", "initial green value", "initial blue value", "initial rotation angle",
        "final time",
        "final x-coordinate", "final y-coordinate",
        "final width", "final height",
        "final red value", "final green value", "final blue value", "final rotation angle"
    };
    int[] vals = new int[18];
    String name;
    if (s.hasNext()) {
    ...
    if (containsRotation) {
      builder.addMotion(name, vals[0],
          new AttributeImpl.Builder(new Position2D(vals[1], vals[2]), vals[3], vals[4],
              new Color(vals[5], vals[6], vals[7])).setAngle(vals[8]).build(), vals[9],
          new AttributeImpl.Builder(new Position2D(vals[10], vals[11]), vals[12], vals[13],
              new Color(vals[14], vals[15], vals[16])).setAngle(vals[17]).build());
    } else {
  ...
  private static <Doc> void readLayer(Scanner s, AnimationBuilder<Doc> builder) {
    if (s.hasNextInt()) {
      ...
      throw new IllegalStateException("Expected an integer, but there was no integer");
    }
  }
